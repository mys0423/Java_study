...
//    	"김"씨 성을 가진 유저만 출력
//      users.stream().filter((user) -> user.getName().startsWith("김")).forEach(System.out::println);
...
//    	"자획기"
//    	"관방소"    
      IsThrity isThrity = (user) -> user.getAge() >= 30 && user.getAge() <= 39;
      MyReverse myReverse = (user) -> new StringBuilder(user.getJob()).reverse().toString();
      
      users.stream().filter(isThrity::isThrity).map(myReverse::myReverse).forEach(System.out::println);
   }
}
